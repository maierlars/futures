...
static unsigned num_threads = std::thread::hardware_concurrency() * 2;
static constexpr unsigned num_rounds = 100000;

template <typename Tag>
struct FuzzerTests : testing::Test {
  struct alignas(64) slot {
    std::shared_ptr<mellon::future<int, Tag>> future;
    ...
    for (unsigned i = 0; i < num_threads / 2; ++i) {
      threads.emplace_back(std::thread([&producer_idx, &slots, i, num_slots]() {
        ...
        for (unsigned r = 0; r < num_rounds; ++r) {
          auto [f, p] = mellon::make_promise<int, Tag>();
          ...
          slots[idx].future = std::make_shared<mellon::future<int, Tag>>(std::move(f));
          ...
          std::move(p).fulfill(idx * (slot_round + 1));
        }
      }));
    }
...
TYPED_TEST(FuzzerTests, finally) {
  this->run_fuzz([](auto future, unsigned idx, unsigned slot_round, auto& num_resolved_futures) {
    ...
    auto f2 = future;
    std::move(*future)
      .and_then([slot_round](unsigned x) noexcept { return x / slot_round; })
